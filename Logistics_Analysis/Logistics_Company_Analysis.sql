...
-- Identify the customer name and the package description for all customers and all shipments. 

SELECT customers.customer_name
, shipments.package_description
FROM customers FULL JOIN shipments
ON customers.customer_id = shipments.customer_id
;

-- Display the shipment id, shipping address, and weight for all shipments weighting 3 or more kilograms. order the results from heaviest to lightest.

SELECT shipments.shipment_id
, customers.shipping_address
, shipments.weight_kg
FROM customers RIGHT JOIN shipments
ON customers.customer_id - shipments.customer_id
...
-- Display the billing address, shipping address, and shipping id for all customers whose billing address is in texas (billing address containts "TX")

SELECT customers.customer_name
,customers.billing_address
, customers.shipping_address
, shipments.shipment_id
FROM customers LEFT JOIN shipments
ON customers.customer_id = shipments.customer_id
